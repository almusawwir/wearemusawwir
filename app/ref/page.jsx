  ...
  return (
    <div className="relative min-h-screen w-full bg-[#0A0A0B] text-[#F5F3EF] font-sans pb-32">
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .color-swatch:hover { transform: scale(1.15) translateY(-2px); box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.5); z-index: 10; relative; }
      `}} />

      {/* Navigation */}
      <div className="absolute top-8 left-4 md:left-8 z-50">
        <a href="/" className="bg-white/10 backdrop-blur px-4 py-2 rounded-full font-sans text-[10px] uppercase tracking-widest font-bold hover:bg-white hover:text-[#0A0A0B] transition-all shadow-[0_4px_20px_rgba(0,0,0,0.3)] text-white border border-white/10">
          ← Return Home
        </a>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-8 pt-32 space-y-16">
        ...
        <div className="text-center space-y-4">
          <h2 className="font-serif text-2xl md:text-3xl text-white tracking-[0.3em] uppercase">3AM Ideas</h2>
          <div className="w-16 h-px bg-[#FF2D78] mx-auto"></div>
        </div>

        {/* Header */}
        <header className="text-center space-y-4 max-w-2xl mx-auto">
          <div className="inline-flex items-center justify-center p-4 bg-white/[0.05] backdrop-blur-md rounded-full shadow-sm mb-2 text-[#FF2D78] border border-white/10">
          ...
          <h1 className="font-serif italic text-5xl md:text-6xl text-white leading-tight">Art Fundamentals</h1>
          <p className="font-serif text-xl md:text-2xl text-white/60">An interactive guide to colour theory, shading, and visual references.</p>
        </header>
        ...
        <section className="bg-white/[0.04] backdrop-blur-md rounded-[2.5rem] p-6 md:p-10 border border-white/10 shadow-xl shadow-black/40">
          <div className="mb-8 border-b border-white/10 pb-6 text-center md:text-left">
            <span className="font-sans text-[10px] uppercase tracking-[0.3em] font-bold text-[#FF2D78]">Chapter 1</span>
            <h2 className="font-serif text-4xl text-white mt-2">Colour Mixing</h2>
            <p className="font-sans text-sm text-white/60 mt-2">Start with the 3 primary colours: <span className="font-bold text-red-400">Red</span>, <span className="font-bold text-yellow-300">Yellow</span>, and <span className="font-bold text-blue-400">Blue</span>.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-4">
            ...
            {/* Special Row: Brown */}
            <div className="flex items-center gap-3 w-full bg-white/[0.03] p-3 rounded-2xl md:col-span-2 group hover:bg-white/[0.08] transition-colors duration-300">
              ...
              <span className="font-serif italic text-sm text-white/50">(a little)</span>
              <EqualIcon />
              <div className="w-12 h-12 rounded-full bg-[#8B4513] shadow-md ring-4 ring-amber-100/20 color-swatch"></div>
              <span className="font-serif text-lg ml-2 text-[#c8814a]">Brown</span>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white/[0.04] backdrop-blur-md rounded-[2.5rem] p-6 md:p-10 border border-white/10 shadow-xl shadow-black/40">
          <div className="mb-8 border-b border-white/10 pb-6 text-center md:text-left">
            <span className="font-sans text-[10px] uppercase tracking-[0.3em] font-bold text-[#4DA3FF]">Chapter 2</span>
            <h2 className="font-serif text-4xl text-white mt-2">Shades and Tints</h2>
            <p className="font-sans text-sm text-white/60 mt-2">Observe how adding White lightens (Tints) and Black deepens (Shades) the base colour.</p>
          </div>
          
          <div className="space-y-12">
            {/* Tints */}
            <div>
              <h3 className="font-sans text-[12px] uppercase tracking-[0.2em] font-bold text-white mb-6 flex items-center gap-3">
                <span className="w-6 h-6 rounded-full bg-white border border-white/20 shadow-sm block"></span>
                Adding White — Tints
              </h3>
            ...
            <div>
              <h3 className="font-sans text-[12px] uppercase tracking-[0.2em] font-bold text-white mb-6 flex items-center gap-3">
                <span className="w-6 h-6 rounded-full bg-[#0A0A0B] border border-white/20 block"></span>
                Adding Black — Shades
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="rounded-2xl overflow-hidden border border-white/10 flex flex-col group hover:shadow-lg transition-all duration-300 relative">
            ...
            <div className="pt-8 border-t border-white/10">
              <h3 className="font-sans text-[12px] uppercase tracking-[0.2em] font-bold text-white mb-6 text-center md:text-left">Warm vs Cool</h3>
        ...
        <section className="bg-white/[0.04] backdrop-blur-md rounded-[2.5rem] p-6 md:p-10 border border-white/10 shadow-xl shadow-black/40">
          <div className="mb-10 text-center space-y-3">
            <span className="font-sans text-[10px] uppercase tracking-[0.3em] font-bold text-white/50">Chapter 3</span>
            <h2 className="font-serif italic text-4xl md:text-5xl text-white">Visual References</h2>
            <p className="font-sans text-sm text-white/60 max-w-xl mx-auto">A curated collection of visual guides to inspire your next piece on canvas.</p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
            {referenceImages.map((src, index) => (
              <div 
                ...
                className="group relative aspect-[4/5] bg-white/5 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.3)] border-[6px] md:border-[10px] border-white/10 overflow-hidden cursor-pointer transform hover:-translate-y-2 transition-all duration-500 hover:shadow-[0_20px_40px_rgb(0,0,0,0.5)]"
              >
                ...
                <div className="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-20 pointer-events-none flex items-center justify-center">
                  <div className="bg-white/90 backdrop-blur text-[#0A0A0B] rounded-full w-10 h-10 flex items-center justify-center transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
...
function MixRow({ c1, c2, result, name }) {
  return (
    <div className="flex items-center gap-3 w-full bg-white/[0.03] p-3 rounded-2xl group hover:bg-white/[0.08] transition-colors duration-300">
      ...
      <EqualIcon />
      <div className={`w-12 h-12 rounded-full ${result} shadow-md ring-4 ring-white/10 color-swatch`}></div>
      <span className="font-serif text-lg ml-2 text-white group-hover:text-[#FF2D78] transition-colors">{name}</span>
    </div>
  );
}
...
function TintCard({ base, soft, light, pale, name, darkText = false }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-white/10 flex flex-col group hover:shadow-lg transition-all duration-300">
