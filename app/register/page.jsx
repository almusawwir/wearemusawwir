...
// ── CONNECTED GOOGLE SHEETS CSV URLs ──
const FORMS_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTSSCmEDqxpPn1OEzXR3geUaynoeGhrswVO5xf8zKETC8xOq1oimP1SiapOAsSPY_nEMTHoDeacTgKC/pub?gid=521115739&single=true&output=csv"; 
const EVENTS_CSV_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTSSCmEDqxpPn1OEzXR3geUaynoeGhrswVO5xf8zKETC8xOq1oimP1SiapOAsSPY_nEMTHoDeacTgKC/pub?gid=0&single=true&output=csv";
...
// ── JITTER FIX: Static background never re-renders when typing ──
...
function RegisterContent() {
  ...
  // Basic required fields that Razorpay needs are initialized here
  const [formData, setFormData] = useState({
    name: '', email: '', whatsapp: '', consent: false
  });
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        if (!cachedEvents) cachedEvents = await fetchCSV(EVENTS_CSV_URL);
        const foundEvent = cachedEvents.find(e => e.id && e.id.trim().toLowerCase() === eventId.toLowerCase());
        
        if (foundEvent) {
          setEventDetails({
            title: foundEvent.title, date: foundEvent.date, time: foundEvent.time,
            price: foundEvent.price || '999', groupPrice: foundEvent.group_price || foundEvent.price || '999',
          });
        }

        // Fetch row-based forms
        if (!cachedForms && FORMS_CSV_URL.includes('http')) cachedForms = await fetchCSV(FORMS_CSV_URL);
        
        if (cachedForms) {
          // Find the specific event form, or fallback to 'default'
          let formRow = cachedForms.find(f => f.id?.trim().toLowerCase() === eventId.toLowerCase());
          if (!formRow) formRow = cachedForms.find(f => f.id?.trim().toLowerCase() === 'default');

          if (formRow) {
            const parsedFields = [];
            
            // Dynamically scan the row for our specific prefixes
            Object.keys(formRow).forEach(key => {
              const val = formRow[key]?.trim();
              if (!val) return; // skip empty cells

              if (key.startsWith('req_q_')) {
                parsedFields.push({ id: key, label: val, type: 'text', required: true });
              } 
              else if (key.startsWith('opt_q_')) {
                parsedFields.push({ id: key, label: val, type: 'textarea', required: false });
              } 
              else if (key.startsWith('mcq_')) {
                const num = key.split('_')[1]; // extracts '01' from 'mcq_01'
                const optionsKey = `options_${num}`;
                if (formRow[optionsKey]) {
                  parsedFields.push({ 
                    id: key, label: val, type: 'radio', 
                    options: formRow[optionsKey].split(',').map(o => o.trim()), 
                    required: true 
                  });
                }
              }
            ...
            // Sort fields alphabetically by their ID to maintain consistent order
            parsedFields.sort((a, b) => a.id.localeCompare(b.id));
  ...
  const totalAmount = (ticketCount === 1 ? parseInt(eventDetails?.price || 999) : parseInt(eventDetails?.groupPrice || 999)) * ticketCount;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const options = {
        ...
        name: 'Al-Musawwir', description: `${ticketCount}x Ticket(s)`,
        order_id: data.order.id,

        handler: async function (razorpayResponse) {
          setIsSuccessLoading(true);
          try {
            // Send everything to your backend API
            await fetch('/api/save-data', {
              method: 'POST', headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                eventId,
                ...formData, // Dynamically includes all custom form inputs
                paymentId: razorpayResponse.razorpay_payment_id,
                ticketCount, totalPaid: totalAmount,
              }),
            });
            ...
            alert('Payment successful, but trouble saving form. WhatsApp us Payment ID: ' + razorpayResponse.razorpay_payment_id);
          }
        },
      ...
      paymentObject.open();
      paymentObject.on('payment.failed', () => { setIsProcessing(false); alert('Payment Failed. Please try again.'); });

    } catch (error) {
      setIsProcessing(false); alert('Something went wrong. Please try again.');
    }
  };

  // Renders the dynamic fields based on their type
  const renderCustomField = (field, index) => {
    // Rotating colors for premium UI feel
    const colors = ['text-[#004E98]', 'text-[#E24E7A]', 'text-[#F9A03F]', 'text-[#FF6B35]'];
    const colorClass = colors[index % colors.length];
    
    // We start numbering custom questions at 2, because "The Basics" is Section 1
    const questionNumber = index + 2;

    if (field.type === 'radio') {
      return (
        <div key={field.id} className="space-y-6">
          <h2 className={`font-sans text-[11px] uppercase tracking-[0.3em] font-bold ${colorClass} border-b border-[#1A1817]/10 pb-2`}>{questionNumber}. {field.label} {field.required ? '*' : ''}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
            {field.options.map((option) => (
              <label key={option} className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all ${formData[field.id] === option ? 'border-[#E24E7A] bg-[#E24E7A]/5' : 'border-[#1A1817]/10 bg-white/40 hover:bg-white/70'}`}>
                <input type="radio" name={field.id} value={option} required={field.required} onChange={handleChange} checked={formData[field.id] === option} className="w-4 h-4 text-[#E24E7A] focus:ring-[#E24E7A]" />
    ...
    if (field.type === 'textarea') {
      return (
        <div key={field.id} className="space-y-6">
          <h2 className={`font-sans text-[11px] uppercase tracking-[0.3em] font-bold ${colorClass} border-b border-[#1A1817]/10 pb-2`}>{questionNumber}. {field.label} {field.required ? '*' : '(Optional)'}</h2>
          <textarea name={field.id} required={field.required} value={formData[field.id] || ''} onChange={handleChange} rows="3"
    ...
    return (
      <div key={field.id} className="space-y-6">
        <h2 className={`font-sans text-[11px] uppercase tracking-[0.3em] font-bold ${colorClass} border-b border-[#1A1817]/10 pb-2`}>{questionNumber}. {field.label} {field.required ? '*' : ''}</h2>
        <input type="text" name={field.id} required={field.required} value={formData[field.id] || ''} onChange={handleChange}
          className="bg-white/50 border border-[#1A1817]/20 rounded-xl w-full px-4 py-3 font-sans text-base text-[#1A1817] focus:outline-none focus:border-[#004E98] focus:bg-white transition-all placeholder:text-[#1A1817]/30"
    ...
    <div className="relative min-h-screen w-full bg-[#F7F5F0] text-[#1A1817] font-sans antialiased selection:bg-[#FF6B35] selection:text-white flex justify-center py-12 px-4 md:px-6">
      ...
      {isSuccessLoading && (
        <div className="fixed inset-0 z-[100] bg-[#F7F5F0]/90 backdrop-blur-md flex flex-col items-center justify-center">
          <div className="relative w-20 h-20 mb-8"><div className="absolute inset-0 border-4 border-[#1A1817]/10 rounded-full"></div><div className="absolute inset-0 border-4 border-[#FF6B35] rounded-full border-t-transparent animate-spin"></div></div>
      ...
      <div className="w-full max-w-2xl relative z-10 flex flex-col gap-8">
        ...
        <div className="glass-card rounded-[2rem] md:rounded-[2.5rem] shadow-2xl shadow-[#1A1817]/5 overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="p-8 md:p-10 flex flex-col gap-8">
            ...
            {/* SECTION 1: THE BASICS (Always Required for Razorpay) */}
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  ...
                  <input type="text" name="name" required value={formData.name} onChange={handleChange} className="bg-white/50 border border-[#1A1817]/20 rounded-xl px-4 py-3 font-serif text-lg text-[#1A1817] focus:outline-none focus:border-[#004E98] focus:bg-white transition-all" placeholder="Aham Brahmasmi" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="font-sans text-xs font-bold text-[#1A1817] uppercase tracking-wider">WhatsApp Number *</label>
                  <input type="tel" name="whatsapp" required value={formData.whatsapp} onChange={handleChange} className="bg-white/50 border border-[#1A1817]/20 rounded-xl px-4 py-3 font-sans text-base text-[#1A1817] focus:outline-none focus:border-[#004E98] focus:bg-white transition-all" placeholder="+91 00000 00000" />
                </div>
                <div className="flex flex-col gap-2 md:col-span-2">
                  <label className="font-sans text-xs font-bold text-[#1A1817] uppercase tracking-wider">Email Address *</label>
                  <input type="email" name="email" required value={formData.email} onChange={handleChange} className="bg-white/50 border border-[#1A1817]/20 rounded-xl px-4 py-3 font-sans text-base text-[#1A1817] focus:outline-none focus:border-[#004E98] focus:bg-white transition-all" placeholder="artist@example.com" />
                </div>
              </div>
            </div>
            ...
            <div className="pt-4">
              <label className="flex items-start gap-3 cursor-pointer group">
                ...
                <span className="font-sans text-sm text-[#5C5855] group-hover:text-[#1A1817] transition-colors">I agree to the <Link href="/terms" target="_blank" className="font-bold text-[#1A1817] underline hover:text-[#FF6B35]">Terms & Guidelines</Link>.</span>
              </label>
            </div>
            ...
            <div className="pt-6 border-t border-[#1A1817]/10">
              ...
              <div className="flex items-center justify-between bg-white/60 p-4 rounded-2xl border border-[#1A1817]/10 mb-4 shadow-sm">
                <div>
                  ...
                  <span className="font-serif text-lg text-[#1A1817] leading-none">{ticketCount === 1 ? `₹${eventDetails?.price || '999'} per person` : `Group Rate: ₹${eventDetails?.groupPrice || '899'} per person`}</span>
                </div>
                <div className="flex items-center bg-[#1A1817] rounded-full p-1 gap-4 text-white">
                  <button onClick={decrement} disabled={ticketCount <= 1 || isProcessing} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-white/20 disabled:opacity-30 transition-colors"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 12H4"></path></svg></button>
                  <span className="font-sans font-bold w-4 text-center">{ticketCount}</span>
                  <button onClick={increment} disabled={ticketCount >= maxTickets || isProcessing} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-white/20 disabled:opacity-30 transition-colors"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"></path></svg></button>
                </div>
              </div>

              {/* URGENCY BANNER UNDER COUNTER */}
              {showCountdown && (
                <div className="flex items-center gap-2 mb-4 px-1">
                  ...
                  <p className="font-sans text-[11px] font-bold uppercase tracking-wider text-[#DC2626]">
                    Only {ticketsLeftParam} ticket{ticketsLeftParam === 1 ? '' : 's'} remaining
                  </p>
                </div>
              )}

              <button disabled={isProcessing || isLoading} type="submit" className="w-full bg-[#1A1817] disabled:bg-[#5C5855] disabled:cursor-not-allowed text-white font-sans text-sm uppercase tracking-[0.2em] font-bold py-5 px-8 rounded-xl hover:bg-[#FF6B35] transition-all hover:shadow-xl hover:-translate-y-1 flex items-center justify-between group">
                <span>{isProcessing ? 'Processing...' : `Secure ${ticketCount} Ticket${ticketCount > 1 ? 's' : ''}`}</span>
                {!isProcessing && <span className="flex items-center gap-3">Pay ₹{totalAmount}</span>}
              </button>
